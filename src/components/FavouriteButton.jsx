...
import { Bookmark, BookmarkCheck, LoaderIcon } from "lucide-react";
...
export default function FavouriteButton({disabled, isFav, productId}) {
    ...
    return (
        <Button onClick={async() => {
            ...
            setState(!state)
            setSaving(false);
        }} disabled={disabled} type="submit" className={"text-secondary bg-transparent border border-secondary hover:bg-secondary hover:text-white disabled:border-neutral-200 disabled:text-neutral-200 disabled:hover:bg-transparent"}><div className=" flex items-center gap-1 ">{saving ? "Saving..." : <Bookmark fill={state ? "rgb(215 18 67)" : "none"}/>}</div></Button>
    )
}